/**
 * esbuild configuration for building the paperplaneai CLI for npm.
 *
 * Bundles all workspace packages (@paperplaneai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @paperplaneai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@paperplaneai/")) {
      externals.add(name);
    }
  }
