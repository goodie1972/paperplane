...
const JWT_SECRET_ENV_KEY = "PAPERPLANE_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# Paperplane environment variables",
    "# Generated by Paperplane CLI commands",
...
export function resolvePaperplaneEnvFile(configPath?: string): string {
...
export function loadPaperplaneEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergePaperplaneEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readPaperplaneEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writePaperplaneEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergePaperplaneEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readPaperplaneEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writePaperplaneEnvEntries(next, filePath);
  return next;
}
