...
function createTempPath(name: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "paperplane-cli-common-"));
...
describe("resolveCommandContext", () => {
  beforeEach(() => {
    ...
    delete process.env.PAPERPLANE_API_URL;
    delete process.env.PAPERPLANE_API_KEY;
    delete process.env.PAPERPLANE_COMPANY_ID;
  });
