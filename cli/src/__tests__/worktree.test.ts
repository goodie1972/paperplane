...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("paperplane-pr-432")).toBe(
      path.resolve(os.homedir(), "paperplane-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("paperplane/pr-432")).toThrow(
  ...
  it("builds isolated config and env paths for a worktree", () => {
    ...
    expect(env.PAPERPLANE_HOME).toBe(path.resolve("/tmp/paperclip-worktrees"));
    expect(env.PAPERPLANE_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.PAPERPLANE_IN_WORKTREE).toBe("true");
    expect(env.PAPERPLANE_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.PAPERPLANE_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export PAPERPLANE_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    ...
    const originalInlineMasterKey = process.env.PAPERPLANE_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.PAPERPLANE_SECRETS_MASTER_KEY_FILE;
    try {
      delete process.env.PAPERPLANE_SECRETS_MASTER_KEY;
      delete process.env.PAPERPLANE_SECRETS_MASTER_KEY_FILE;
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.PAPERPLANE_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.PAPERPLANE_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.PAPERPLANE_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          PAPERPLANE_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    ...
    const originalJwtSecret = process.env.PAPERPLANE_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.PAPERPLANE_AGENT_JWT_SECRET = "worktree-shared-secret";
      ...
      const envPath = path.join(repoRoot, ".paperplane", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("PAPERPLANE_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("PAPERPLANE_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/PAPERPLANE_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.PAPERPLANE_AGENT_JWT_SECRET;
      } else {
        process.env.PAPERPLANE_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("defaults the seed source config to the current repo-local Paperclip config", () => {
    ...
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".paperplane", "config.json");
    const originalCwd = process.cwd();
    const originalPaperclipConfig = process.env.PAPERPLANE_CONFIG;

    try {
      fs.mkdirSync(path.dirname(localConfigPath), { recursive: true });
      fs.writeFileSync(localConfigPath, JSON.stringify(buildSourceConfig()), "utf8");
      delete process.env.PAPERPLANE_CONFIG;
      process.chdir(repoRoot);
      ...
      if (originalPaperclipConfig === undefined) {
        ...
        process.env.PAPERPLANE_CONFIG = originalPaperclipConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    ...
    try {
      ...
      expect(fs.existsSync(path.join(worktreePath, ".paperplane", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".paperplane", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
