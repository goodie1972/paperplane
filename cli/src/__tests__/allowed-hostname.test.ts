...
function createTempConfigPath() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "paperplane-allowed-hostname-"));
...
function writeBaseConfig(configPath: string) {
  const base: PaperclipConfig = {
    ...
    database: {
      ...
      embeddedPostgresDataDir: "/tmp/paperplane-db",
      embeddedPostgresPort: 54329,
      backup: {
        enabled: true,
        intervalMinutes: 60,
        retentionDays: 30,
        dir: "/tmp/paperplane-backups",
      },
    },
    logging: {
      mode: "file",
      logDir: "/tmp/paperplane-logs",
    },
    server: {
    ...
    storage: {
      provider: "local_disk",
      localDisk: { baseDir: "/tmp/paperplane-storage" },
      s3: {
        bucket: "paperplane",
        region: "us-east-1",
        prefix: "",
    ...
    secrets: {
      ...
      strictMode: false,
      localEncrypted: { keyFilePath: "/tmp/paperplane-secrets/master.key" },
    },
  };
