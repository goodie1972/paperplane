...
import {
  ...
  runDatabaseBackup,
  runDatabaseRestore,
} from "@paperplaneai/db";
import type { Command } from "commander";
import { ensureAgentJwtSecret, loadPaperplaneEnvFile, mergePaperplaneEnvEntries, readPaperplaneEnvEntries, resolvePaperplaneEnvFile } from "../config/env.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.PAPERPLANE_CONFIG;
...
const WORKTREE_NAME_PREFIX = "paperplane-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.PAPERPLANE_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.PAPERPLANE_WORKTREE_START_POINT) ?? undefined;
}
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.paperplane"));
...
export function copySeededSecretsKey(input: {
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.PAPERPLANE_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.PAPERPLANE_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.PAPERPLANE_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.PAPERPLANE_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  ...
  const sourceEnvFile = resolvePaperplaneEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readPaperplaneEnvEntries(sourceEnvFile);
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readPaperplaneEnvEntries(resolvePaperplaneEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.PAPERPLANE_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.PAPERPLANE_AGENT_JWT_SECRET);
  mergePaperplaneEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { PAPERPLANE_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadPaperplaneEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run Paperplane inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}
...
export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" paperplaneai worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printPaperplaneCliBanner();
  p.intro(pc.bgCyan(pc.black(" paperplaneai worktree:make ")));
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printPaperplaneCliBanner();
  p.intro(pc.bgCyan(pc.black(" paperplaneai worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolvePaperplaneEnvFile(configPath);
  const envEntries = readPaperplaneEnvEntries(envPath);
  const out = {
    PAPERPLANE_CONFIG: configPath,
    ...(envEntries.PAPERPLANE_HOME ? { PAPERPLANE_HOME: envEntries.PAPERPLANE_HOME } : {}),
    ...(envEntries.PAPERPLANE_INSTANCE_ID ? { PAPERPLANE_INSTANCE_ID: envEntries.PAPERPLANE_INSTANCE_ID } : {}),
    ...(envEntries.PAPERPLANE_CONTEXT ? { PAPERPLANE_CONTEXT: envEntries.PAPERPLANE_CONTEXT } : {}),
    ...envEntries,
  };
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local Paperplane instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated Paperplane instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with paperplane- if needed (created at ~/paperplane-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: PAPERPLANE_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: PAPERPLANE_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source PAPERPLANE_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local Paperplane instance")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with paperplane- if needed")
