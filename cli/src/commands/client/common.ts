...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Paperplane config file")
    .option("-d, --data-dir <path>", "Paperplane data directory root (isolates state from ~/.paperplane)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Paperplane API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.PAPERPLANE_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const apiKey =
    options.apiKey?.trim() ||
    process.env.PAPERPLANE_API_KEY?.trim() ||
    readKeyFromProfileEnv(profile);

  const companyId =
    options.companyId?.trim() ||
    process.env.PAPERPLANE_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set PAPERPLANE_COMPANY_ID, or set context profile companyId via `paperplaneai context set`.",
    );
  }
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.PAPERPLANE_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.PAPERPLANE_SERVER_PORT || "");
