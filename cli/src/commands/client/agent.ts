...
import type { Agent } from "@paperplaneai/shared";
import {
  removeMaintainerOnlySkillSymlinks,
  resolvePaperclipSkillsDir,
} from "@paperplaneai/adapter-utils/server-utils";
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export PAPERPLANE_API_URL='${escaped(input.apiBase)}'`,
    `export PAPERPLANE_COMPANY_ID='${escaped(input.companyId)}'`,
    `export PAPERPLANE_AGENT_ID='${escaped(input.agentId)}'`,
    `export PAPERPLANE_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Paperplane skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Paperplane skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            ...
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Paperplane skills directory. Expected ./skills in the repo checkout.",
              );
            }
