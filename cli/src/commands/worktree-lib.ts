...
export const DEFAULT_WORKTREE_HOME = "~/.paperplane-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".paperplane");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "paperplane",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    PAPERPLANE_HOME: paths.homeDir,
    PAPERPLANE_INSTANCE_ID: paths.instanceId,
    PAPERPLANE_CONFIG: paths.configPath,
    PAPERPLANE_CONTEXT: paths.contextPath,
    PAPERPLANE_IN_WORKTREE: "true",
    ...(branding?.name ? { PAPERPLANE_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { PAPERPLANE_WORKTREE_COLOR: branding.color } : {}),
  };
}
