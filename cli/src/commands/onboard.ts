...
import { printPaperplaneCliBanner } from "../utils/banner.js";
...
const ONBOARD_ENV_KEYS = [
  "PAPERPLANE_PUBLIC_URL",
  "DATABASE_URL",
  "PAPERPLANE_DB_BACKUP_ENABLED",
  "PAPERPLANE_DB_BACKUP_INTERVAL_MINUTES",
  "PAPERPLANE_DB_BACKUP_RETENTION_DAYS",
  "PAPERPLANE_DB_BACKUP_DIR",
  "PAPERPLANE_DEPLOYMENT_MODE",
  "PAPERPLANE_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "PAPERPLANE_ALLOWED_HOSTNAMES",
  "PAPERPLANE_AUTH_BASE_URL_MODE",
  "PAPERPLANE_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "PAPERPLANE_STORAGE_PROVIDER",
  "PAPERPLANE_STORAGE_LOCAL_DIR",
  "PAPERPLANE_STORAGE_S3_BUCKET",
  "PAPERPLANE_STORAGE_S3_REGION",
  "PAPERPLANE_STORAGE_S3_ENDPOINT",
  "PAPERPLANE_STORAGE_S3_PREFIX",
  "PAPERPLANE_STORAGE_S3_FORCE_PATH_STYLE",
  "PAPERPLANE_SECRETS_PROVIDER",
  "PAPERPLANE_SECRETS_STRICT_MODE",
  "PAPERPLANE_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const instanceId = resolvePaperplaneInstanceId();
  const defaultStorage = defaultStorageConfig();
  const defaultSecrets = defaultSecretsConfig();
  const databaseUrl = process.env.DATABASE_URL?.trim() || undefined;
  const publicUrl =
    process.env.PAPERPLANE_PUBLIC_URL?.trim() ||
    process.env.PAPERPLANE_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.PAPERPLANE_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.PAPERPLANE_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.PAPERPLANE_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.PAPERPLANE_ALLOWED_HOSTNAMES
    ? process.env.PAPERPLANE_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.PAPERPLANE_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.PAPERPLANE_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.PAPERPLANE_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.PAPERPLANE_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.PAPERPLANE_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.PAPERPLANE_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.PAPERPLANE_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.PAPERPLANE_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.PAPERPLANE_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.PAPERPLANE_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.PAPERPLANE_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.PAPERPLANE_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.PAPERPLANE_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.PAPERPLANE_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.PAPERPLANE_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "PAPERPLANE_DEPLOYMENT_EXPOSURE",
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  printPaperplaneCliBanner();
  p.intro(pc.bgCyan(pc.black(" paperplaneai onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolvePaperplaneInstanceId());
  p.log.message(
    pc.dim(
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@paperplaneai/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `paperplaneai doctor`"));
      }
    }
  ...
  if (jwtSecret.created) {
    p.log.success(`Created ${pc.cyan("PAPERPLANE_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  } else if (process.env.PAPERPLANE_AGENT_JWT_SECRET?.trim()) {
    p.log.info(`Using existing ${pc.cyan("PAPERPLANE_AGENT_JWT_SECRET")} from environment`);
  } else {
    p.log.info(`Using existing ${pc.cyan("PAPERPLANE_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  }
  ...
  p.note(
    [
      ...
      "Agent auth: PAPERPLANE_AGENT_JWT_SECRET configured",
    ].join("\n"),
    "Configuration saved",
  );
  ...
  p.note(
    [
      `Run: ${pc.cyan("paperplaneai run")}`,
      `Reconfigure later: ${pc.cyan("paperplaneai configure")}`,
      `Diagnose setup: ${pc.cyan("paperplaneai doctor")}`,
    ].join("\n"),
    "Next commands",
  );
  ...
  if (shouldRunNow && !opts.invokedByRun) {
    process.env.PAPERPLANE_OPEN_ON_LISTEN = "true";
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("paperplaneai run")}`,
        `Then: ${pc.cyan("paperplaneai auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
