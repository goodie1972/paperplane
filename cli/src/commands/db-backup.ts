...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@paperplaneai/db";
import {
  expandHomePrefix,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" paperplane db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolvePaperplaneInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "paperplane";
