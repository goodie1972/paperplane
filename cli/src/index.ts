...
const DATA_DIR_OPTION_HELP =
  "Paperplane data directory root (isolates state from ~/.paperplane)";

program
  .name("paperplaneai")
  .description("Paperplane CLI — setup, diagnose, and configure your instance")
  .version("0.3.1");
...
program.hook("preAction", (_thisCommand, actionCommand) => {
  ...
  loadPaperplaneEnvFile(options.config);
});
...
program
  ...
  .option("--run", "Start Paperplane immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your Paperplane setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Paperplane")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Paperplane server API")
