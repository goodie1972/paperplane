...
## Scope
This plan is now **gateway-only**. Paperplane supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Paperplane skill.
...
```json
{
  "paperplaneApiUrl": "http://host.docker.internal:3100",
...
3. In `/new` OpenClaw session, OpenClaw can still create a Paperplane task.
...
```bash
pnpm --filter @paperplaneai/server test:run -- openclaw-gateway
pnpm --filter @paperplaneai/server typecheck
pnpm --filter @paperplaneai/ui typecheck
pnpm --filter paperplaneai typecheck
```
