...
This document describes how `@paperplaneai/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into Paperplane logs/transcript parsing
...
- required fields:
  ...
  - `idempotencyKey` (Paperplane `runId`)
