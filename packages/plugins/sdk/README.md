# `@paperplaneai/plugin-sdk`

Official TypeScript SDK for Paperplane plugin authors.

- **Worker SDK:** `@paperplaneai/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@paperplaneai/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@paperplaneai/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@paperplaneai/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@paperplaneai/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@paperplaneai/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@paperplaneai/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@paperplaneai/plugin-sdk/ui/hooks` | Hooks only |
| `@paperplaneai/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@paperplaneai/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@paperplaneai/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@paperplaneai/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@paperplaneai/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@paperplaneai/plugin-sdk/types` | Worker context and API types (advanced) |
...
```bash
pnpm add @paperplaneai/plugin-sdk
```
...
- Plugin UI bundles run as same-origin JavaScript inside the main Paperplane app. They can call ordinary Paperplane HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Paperplane instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@paperplaneai/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@paperplaneai/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Paperplane information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@paperplaneai/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@paperplaneai/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@paperplaneai/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@paperplaneai/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@paperplaneai/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@paperplaneai/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@paperplaneai/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@paperplaneai/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@paperplaneai/plugin-sdk/ui";
...
import {
  useHostContext,
  usePluginAction,
} from "@paperplaneai/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@paperplaneai/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@paperplaneai/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@paperplaneai/plugin-sdk/bundlers";
...
```bash
paperplane-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@paperplaneai/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__paperplane__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__paperplane__/events` streams `reload` SSE events on UI build changes
