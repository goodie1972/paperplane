...
Example Paperplane plugin that demonstrates:
...
```bash
pnpm --filter @paperplaneai/plugin-file-browser-example build
pnpm paperplaneai plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm paperplaneai plugin uninstall paperplane-file-browser-example --force
```
...
- Optional: use `paperplane-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
