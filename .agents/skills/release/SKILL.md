...
name: release
description: >
  Coordinate a full Paperplane release across engineering verification, npm,
...
Run the full Paperplane release as a maintainer workflow, not just an npm publish.
...
7. If running through Paperplane, you have issue context for status updates and follow-up task creation.
...
Paperplane now uses this release model:
...
```bash
npm view paperplaneai@canary version
```
...
- any release summary in Paperplane issue context
