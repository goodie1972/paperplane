...
import { PROJECT_COLORS } from "@paperplaneai/shared";
...
export function NewProjectDialog() {
  ...
  async function handleSubmit() {
    ...
    if (localRequired && !isAbsolutePath(localPath)) {
      setWorkspaceError(t("project.localPathError"));
      return;
    }
    if (repoRequired && !isGitHubRepoUrl(repoUrl)) {
      setWorkspaceError(t("project.githubUrlError"));
      return;
    }
    ...
    >
      ...
      >
        ...
        <div className="px-4 pb-3 space-y-3 border-t border-border">
          <div className="pt-3">
            <p className="text-sm font-medium">{t("project.workspaceQuestion")}</p>
            <p className="text-xs text-muted-foreground">{t("project.workspaceHint")}</p>
          </div>
          <div className="grid gap-2 sm:grid-cols-3">
            ...
            >
              ...
              <p className="mt-1 text-xs text-muted-foreground">{t("project.localFolderHint")}</p>
            </button>
            <button
              type="button"
              className={cn(
            ...
            >
              ...
              <p className="mt-1 text-xs text-muted-foreground">{t("project.githubRepoHint")}</p>
            </button>
            <button
              type="button"
              className={cn(
            ...
            >
              ...
              <p className="mt-1 text-xs text-muted-foreground">{t("project.bothHint")}</p>
            </button>
          </div>

          {(workspaceSetup === "local" || workspaceSetup === "both") && (
            <div className="rounded-md border border-border p-2">
              <label className="mb-1 block text-xs text-muted-foreground">{t("project.localFolderLabel")}</label>
          ...
          {(workspaceSetup === "repo" || workspaceSetup === "both") && (
            <div className="rounded-md border border-border p-2">
              <label className="mb-1 block text-xs text-muted-foreground">{t("project.githubRepoLabel")}</label>
              <input
          ...
          <Popover open={goalOpen} onOpenChange={setGoalOpen}>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {selectedGoals.length > 0 ? t("project.addGoal") : t("project.goalLabel")}
              </button>
            </PopoverTrigger>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          {createProject.isError ? (
            <p className="text-xs text-destructive">{t("project.createError")}</p>
          ) : (
            <span />
          )}
          <Button
