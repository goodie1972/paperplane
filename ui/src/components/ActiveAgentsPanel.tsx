...
import type { Issue } from "@paperplaneai/shared";
    ...
    <div className={cn(
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <div className="mt-2 flex items-center gap-2 text-[11px] text-muted-foreground">
              <span>{isActive ? t("agents.liveNow") : run.finishedAt ? `${t("agents.finished")} ${relativeTime(run.finishedAt)}` : `${t("agents.started")} ${relativeTime(run.createdAt)}`}</span>
            </div>
          </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        <RunTranscriptView
          ...
          emptyMessage={hasOutput ? t("agents.waitingTranscript") : isActive ? t("agents.waitingOutput") : t("agents.noTranscript")}
        />
      </div>
    </div>
  );
