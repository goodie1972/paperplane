...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Paperplane data dir to use
  PAPERPLANE_HOST=127.0.0.1    Host passed to the onboarded server
  PAPERPLANE_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/paperplane-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/paperplane-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export PAPERPLANE_HOST="${PAPERPLANE_HOST:-127.0.0.1}"
export PAPERPLANE_PORT="${PAPERPLANE_PORT:-3100}"
export PAPERPLANE_OPEN_ON_LISTEN="${PAPERPLANE_OPEN_ON_LISTEN:-false}"
...
echo "PAPERPLANE_HOST: $PAPERPLANE_HOST"
echo "PAPERPLANE_PORT: $PAPERPLANE_PORT"
...
env \
  ...
  HOST="$PAPERPLANE_HOST" \
  PORT="$PAPERPLANE_PORT" \
  PAPERPLANE_OPEN_ON_LISTEN="$PAPERPLANE_OPEN_ON_LISTEN" \
  pnpm paperplaneai onboard --yes --data-dir "$PC_DATA"
