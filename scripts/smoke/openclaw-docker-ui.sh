...
PAPERPLANE_HOST_PORT="${PAPERPLANE_HOST_PORT:-3100}"
PAPERPLANE_HOST_FROM_CONTAINER="${PAPERPLANE_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.paperplane-openclaw.override.yml"
...
detect_paperplane_base_url() {
  ...
  for candidate in "$PAPERPLANE_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${PAPERPLANE_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${PAPERPLANE_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
paperplane_base_url="$(detect_paperplane_base_url || true)"
...
Paperplane URL for OpenClaw container:
EOF
  if [[ -n "$paperplane_base_url" ]]; then
    cat <<EOF
  $paperplane_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${PAPERPLANE_HOST_PORT}
  (Do not use http://127.0.0.1:${PAPERPLANE_HOST_PORT} inside the container.)
  If Paperplane rejects the host, run on host machine:
    pnpm paperplaneai allowed-hostname host.docker.internal
  Then restart Paperplane and re-run this script.
EOF
  fi
  cat <<EOF
