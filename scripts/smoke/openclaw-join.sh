...
COMPANY_ID="${COMPANY_ID:-${PAPERPLANE_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-paperplane-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-paperplane-openclaw-smoke}"
...
Provide one of:
  PAPERPLANE_AUTH_HEADER=\"Bearer <board-token>\"
  PAPERPLANE_COOKIE=\"<board-session-cookie>\"
...
log "checking Paperplane health"
...
assert_status "200"
if ! grep -q "Paperplane OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.paperplane.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
