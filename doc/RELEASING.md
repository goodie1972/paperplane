# Releasing Paperplane

Maintainer runbook for shipping a full Paperplane release across npm, GitHub, and the website-facing changelog surface.
...
2. **npm** — `paperplaneai` and public workspace packages are published
...
```bash
VERSION=X.Y.Z
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for Paperplane. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
PAPERPLANEAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
```

Users install canaries with:

```bash
npx paperplaneai@canary onboard
```
...
Paperplane uses one release branch per target stable version:
...
./scripts/release-start.sh minor --worktree-dir ../paperplane-release-0.4.0
...
```bash
HOST_PORT=3232 DATA_DIR=./data/release-smoke-canary PAPERPLANEAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable PAPERPLANEAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```
...
PAPERPLANE_PORT=3234 ./scripts/clean-onboard-ref.sh
./scripts/clean-onboard-ref.sh HEAD
```

Minimum checks:

- `npx paperplaneai@canary onboard` installs
...
- [ ] `npm view paperplaneai@latest version` matches the new stable version
