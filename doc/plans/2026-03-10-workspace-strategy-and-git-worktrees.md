...
`PAP-447` asks how Paperplane should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Paperplane instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Paperplane:

- some operators use Paperplane to manage Paperplane and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Paperplane
- Claude and Codex expose different built-in affordances, so Paperplane should not overfit to one tool

## Core Product Decision

Paperplane should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Paperplane
...
- Paperplane should not assume that every adapter can see or use a host filesystem path directly
...
If Paperplane makes worktrees agent-first, it will blur:
...
- Paperplane core resolves and records execution workspace state
...
That gives Paperplane room to support:
...
- Paperplane handles workspace runtime services under the hood using project/workspace policy
...
Once Paperplane is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So Paperplane should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Paperplane cannot infer
...
- when issue moves to done, should Paperplane auto-commit?
- should it auto-push?
...
Paperplane already has the right foundation for a project-first model.
...
For server-spawned local adapters, Paperplane already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Paperplane should mirror directly

Implication:

- for `codex_local`, Paperplane should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Paperplane should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Paperplane should separate:
...
Paperplane should be able to express intentions such as:
...
These are not specific to Paperplane. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".paperplane/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Paperplane-specific `devServer` flag.
...
Paperplane should distinguish between:
...
Paperplane should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Paperplane should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Paperplane
...
For runtime services specifically, Paperplane should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Paperplane-only dev-server flags
...
3. Inject resolved execution workspace details into `context.paperplaneWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Paperplane instance can be reused through env/config injection when appropriate
...
- Paperplane can decide whether to reuse or start a fresh service deterministically
...
3. Test against Paperplane-on-Paperplane workflows first.
...
7. Workspace runtime services are modeled generically, not as Paperplane-specific dev-server toggles.
