...
Agents need API keys to authenticate with Paperplane. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Paperplane
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Paperplane generates a token and passes it directly to the agent
...
`PAPERPLANE_API_URL`, `PAPERPLANE_AGENT_ID`, `PAPERPLANE_COMPANY_ID`. We need to
add a `PAPERPLANE_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Paperplane CLI
...
```
paperplane auth login
# Opens browser -> user confirms -> token stored at ~/.paperplane/credentials
```
...
managed by the Paperplane server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Paperplane.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Paperplane SKILL.md (or a link to it)
   - What information Paperplane needs from the agent (e.g. webhook URL, adapter
...
5. Paperplane stores the pending registration.
...
7. On approval, Paperplane provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Paperplane issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.paperplane.ing/skills/paperplane/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Paperplane should send you heartbeats",
...
1. Generate a Paperplane invite link for the company.
...
4. A Paperplane company member approves the new agent.
5. Paperplane begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `PAPERPLANE_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `paperplaneai auth login` for developer-managed remote agents.                                      |
