...
Paperplane needs to support two very different but equally valid ways of working:
...
Today, Paperplane already has the beginnings of this model:
...
- making Paperplane feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Paperplane users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Paperplane deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Paperplane into a full code review product
...
This object must be recorded explicitly so that Paperplane can:
...
Paperplane should treat PRs as a type of work product linked back to:
...
Paperplane already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Paperplane must be a first-class deployment mode

Paperplane cannot assume that it is running on the same machine as the code.

In cloud deployments, Paperplane may:
...
- `Runtime service`: a process or service Paperplane owns or tracks for a workspace
...
It also lets cloud-hosted Paperplane deployments point at codebases and remotes without pretending the Paperplane host has direct filesystem access.
...
Without an explicit `execution workspace` record, Paperplane has nowhere stable to attach:
...
Paperplane needs a single place to show:
...
- Paperplane-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Paperplane owning the execution host
...
## 1. Paperplane host and execution host must be decoupled
...
without the Paperplane host itself running local git or local preview processes.
...
This is how Paperplane supports:
...
That keeps Paperplane focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Paperplane deployments remain first-class

That is a better fit for Paperplane than either extreme:
