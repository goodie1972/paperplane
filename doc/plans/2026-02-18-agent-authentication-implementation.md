...
- New config/env settings:
  - `PAPERPLANE_AGENT_JWT_SECRET`
  - `PAPERPLANE_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `PAPERPLANE_AGENT_JWT_ISSUER` (default: `paperplane`)
  - `PAPERPLANE_AGENT_JWT_AUDIENCE` (default: `paperplane-api`)
...
1. In:
   ...
   inject `PAPERPLANE_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `PAPERPLANE_API_KEY`, do not overwrite it.
- Continue injecting:
  - `PAPERPLANE_AGENT_ID`
  - `PAPERPLANE_COMPANY_ID`
  - `PAPERPLANE_API_URL`
...
- Local adapters authenticate without manual `PAPERPLANE_API_KEY` config.
