# Paperplane Module System

## Overview

Paperplane's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Paperplane's API, UI, and data model | Yes |
...
modules/
  observability/
    paperplane.module.json     # manifest (required)
    src/
...
### Manifest (`paperplane.module.json`)
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@paperplaneai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@paperplaneai/core";
...
```
1. Scan modules/ directory for paperplane.module.json manifests
...
```jsonc
// paperplane.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@paperplaneai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@paperplaneai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:paperplane/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.paperplane.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm paperplaneai store list                    # browse available modules and templates
pnpm paperplaneai store install <module-id>     # install a module
pnpm paperplaneai store import <template-id>    # import a company template
pnpm paperplaneai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Paperplane instance | `server:started` |
...
4. **Module config** — `paperplane.config.json` with per-module config, env var interpolation
...
10. **`@paperplaneai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
