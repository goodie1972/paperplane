...
Add a single storage subsystem for Paperplane that supports:
...
- API serves attachment bytes through authenticated Paperplane endpoints.
...
- Default local path is under instance root: `~/.paperplane/instances/<instanceId>/data/storage`.
...
- `paperplaneai onboard` writes a valid `storage` config block by default.
- `paperplaneai configure --section storage` can switch between local and s3 modes.
...
- `paperplaneai doctor` reports actionable storage status.
