...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Paperplane plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Paperplane.
...
- package: `@paperplaneai/plugin-kitchen-sink-example`
- plugin id: `paperplane.kitchen-sink-example` or `paperplane-kitchen-sink-example`

Recommendation: use `paperplane-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Paperplane event such as `issue.created`
...
Recommendation: default to a Paperplane-local endpoint or a stable public echo endpoint to avoid flaky docs.
