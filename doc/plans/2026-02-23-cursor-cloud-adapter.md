...
This document defines the V1 design for a Paperplane adapter that integrates with
...
4. synthesized stdout events for Paperplane UI/CLI
...
3. **Agent callback auth to Paperplane** uses a bootstrap exchange flow (no long-lived Paperplane key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Paperplane endpoints, not full SKILL.md prompt injection.
...
- `paperplanePublicUrl` (optional override; else `PAPERPLANE_PUBLIC_URL` env)
...
## Paperplane Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `PAPERPLANE_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.paperplanePublicUrl`
2. `process.env.PAPERPLANE_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Paperplane credentials in prompt text.

Flow:

1. Before launch/follow-up, Paperplane mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Paperplane validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Paperplane API calls.
...
1. Prompt includes a compact instruction to fetch skills from Paperplane.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/paperplane`
   - `GET /api/skills/paperplane-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <paperplanePublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Paperplane run context.
...
5. `paperplanePublicUrl` present and reachable shape-valid
...
- poll interval
- timeout/grace
- paperplane public URL override
- enable webhooks
...
| Skill delivery | local skill dir injection | authenticated fetch from Paperplane skill endpoints |
| Paperplane auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Paperplane context.
3. Add richer PR metadata surfacing in Paperplane UI.
