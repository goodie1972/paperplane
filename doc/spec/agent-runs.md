...
This spec defines how Paperplane actually runs agents while staying runtime-agnostic.
...
1. Paperplane is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Paperplane manages lifecycle, and on exit Paperplane parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `paperplane.skill` (shared Paperplane skill text block)
...
2. Preferred transport is env vars (`PAPERPLANE_*`) injected at runtime.
