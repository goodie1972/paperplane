...
Low-level reference for how Paperplane packages are built for npm.
...
The CLI package, `paperplaneai`, imports code from workspace packages such as:
...
- `npx paperplaneai@canary onboard` can install them explicitly
- `npx paperplaneai onboard` continues to resolve `latest`
