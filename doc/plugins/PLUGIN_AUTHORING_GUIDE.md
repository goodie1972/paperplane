...
This guide describes the current, implemented way to create a Paperplane plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Paperplane app.
...
```bash
pnpm --filter @paperplaneai/create-paperplane-plugin build
node packages/plugins/create-paperplane-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the Paperplane repo:

```bash
pnpm --filter @paperplaneai/create-paperplane-plugin build
node packages/plugins/create-paperplane-plugin/dist/index.js @yourscope/plugin-name \
  --output /absolute/path/to/plugin-repos \
  --sdk-path /absolute/path/to/paperplane/packages/plugins/sdk
```
...
Inside this monorepo, the scaffold uses `workspace:*` for `@paperplaneai/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@paperplaneai/plugin-sdk` from the local Paperplane checkout into a `.paperplane-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Paperplane from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@paperplaneai/plugin-sdk/ui`
