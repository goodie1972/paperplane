# Paperplane Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Paperplane's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Paperplane app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Paperplane HTTP APIs directly.
...
Paperplane plugin design is based on the following assumptions:

1. Paperplane is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Paperplane business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Paperplane core.
...
2. Let plugins add major capabilities without editing Paperplane core.
...
4. Depend on project-local plugin folders such as `.paperplane/plugins`.
...
The single Paperplane deployment an operator installs and controls.

### 5.2 Company

A first-class Paperplane business object inside the instance.
...
A trusted in-process extension loaded directly by Paperplane core.
...
An installable instance-wide extension package loaded through the Paperplane plugin runtime.
...
Paperplane has two extension classes.
...
Paperplane already has a concrete workspace model:
...
Plugins live under the Paperplane instance directory.

Suggested layout:

- `~/.paperplane/instances/default/plugins/package.json`
- `~/.paperplane/instances/default/plugins/node_modules/`
- `~/.paperplane/instances/default/plugins/.cache/`
- `~/.paperplane/instances/default/data/plugins/<plugin-id>/`
...
Paperplane should add CLI commands:

- `pnpm paperplaneai plugin list`
- `pnpm paperplaneai plugin install <package[@version]>`
- `pnpm paperplaneai plugin uninstall <plugin-id>`
- `pnpm paperplaneai plugin upgrade <plugin-id> [version]`
- `pnpm paperplaneai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@paperplane/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface PaperplanePluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
  ...
  minimumPaperplaneVersion?: string;
  capabilities: string[];
  entrypoints: {
    worker: string;
    ui?: string;
...
Plugins may contribute tools that Paperplane agents can use during runs.
...
- Paperplane server starts one worker process per installed plugin
...
Receives one typed Paperplane domain event.
...
export function definePlugin(definition: PluginDefinition): PaperplanePlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: PaperplanePluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@paperplaneai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@paperplaneai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@paperplaneai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@paperplaneai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@paperplaneai/plugin-sdk/ui`
...
The `@paperplaneai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Paperplane secret provider system rather than a plain text input
...
1. Core Paperplane data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Paperplane objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Paperplane product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Paperplane later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Paperplane secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm paperplaneai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Paperplane server. This is a normative requirement, not optional.
...
### 27.1 `@paperplaneai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@paperplaneai/plugin-test-harness";
...
For developing a plugin against a running Paperplane instance:

- The operator installs the plugin from a local path: `pnpm paperplaneai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-paperplane-plugin`) that scaffolds:

- `package.json` with correct `paperplanePlugin` keys
...
- `@paperplane/plugin-workspace-files`
- `@paperplane/plugin-terminal`
- `@paperplane/plugin-git`
- `@paperplane/plugin-linear`
- `@paperplane/plugin-github-issues`
- `@paperplane/plugin-grafana`
- `@paperplane/plugin-runtime-processes`
- `@paperplane/plugin-stripe`
...
- `@paperplaneai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@paperplaneai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@paperplaneai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@paperplaneai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@paperplaneai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@paperplaneai/plugin-sdk/ui`
...
- `@paperplaneai/plugin-test-harness`
- `create-paperplane-plugin` starter template
...
Paperplane should not implement a generic in-process hook bag modeled directly after local coding tools.

Paperplane should implement:
...
That is the complete target design for the Paperplane plugin system.
