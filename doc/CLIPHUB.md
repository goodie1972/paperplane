...
ClipHub is the public registry where people share, discover, and download Paperplane company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Paperplane what a package registry is to a programming language. Paperplane already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Paperplane — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Paperplane instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Paperplane spec. It contains:
...
**Install (fresh start):**
```
paperplane install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Paperplane instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Paperplane, export your company as a template, then publish:

```
paperplane export --template my-company
paperplane publish cliphub my-company
```
...
ClipHub is a **separate service** from Paperplane itself. Paperplane is self-hosted; ClipHub is a hosted registry that Paperplane instances talk to.
...
| **Paperplane CLI** | `paperplaneai install`, `paperplaneai publish`, `paperplaneai cliphub sync` — built into Paperplane |
| **Paperplane UI** | "Browse ClipHub" panel in the Paperplane web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Paperplane) |
| Backend | TypeScript + Hono (consistent with Paperplane) |
| Database | PostgreSQL |
...
4. Run `paperplaneai install cliphub:acme/lean-saas-shop`
5. Paperplane creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Paperplane until it works well
2. Export: `paperplaneai export --template my-agency`
3. Publish: `paperplaneai publish cliphub my-agency`
...
3. Install just that agent: `paperplaneai install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Paperplane

ClipHub is **not required** to use Paperplane. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Paperplane what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `paperplaneai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Paperplane web UI
- [ ] `paperplaneai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Paperplane's own philosophy)
