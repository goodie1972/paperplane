---
name: paperplane-create-plugin
description: >
  Create new Paperplane plugins with the current alpha SDK/runtime. Use when
...
# Create a Paperplane Plugin

Use this skill when the task is to create, scaffold, or document a Paperplane plugin.
...
node packages/plugins/create-paperplane-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Paperplane repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.paperplane-sdk/`:

```bash
pnpm --filter @paperplaneai/create-paperplane-plugin build
node packages/plugins/create-paperplane-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Paperplane from an absolute local path during development
