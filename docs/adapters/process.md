...
- Running a Python script that calls the Paperplane API
...
1. Paperplane spawns the configured command as a child process
2. Standard Paperplane environment variables are injected (`PAPERPLANE_AGENT_ID`, `PAPERPLANE_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the Paperplane API and perform work.
