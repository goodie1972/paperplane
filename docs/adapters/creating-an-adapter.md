...
Build a custom adapter to connect Paperplane to any agent runtime.
...
2. Build environment with `buildPaperplaneEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `paperplaneai run --watch` using `picocolors`.
...
Make Paperplane skills discoverable to your agent runtime without writing to the agent's working directory:
