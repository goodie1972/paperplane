...
Add first-class **documents** to Paperplane as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Paperplane workflow/docs.
...
Update the Paperplane agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/paperplane/SKILL.md`
