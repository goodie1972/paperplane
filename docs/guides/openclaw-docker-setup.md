...
How to get OpenClaw running in a Docker container for local development and testing the Paperplane OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Paperplane includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-paperplane-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a Paperplane host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-paperplane-smoke`)
- `OPENCLAW_RESET_STATE=1` (default) resets smoke agent state on each run to avoid stale auth/session drift
- `PAPERPLANE_HOST_PORT` (default `3100`)
- `PAPERPLANE_HOST_FROM_CONTAINER` (default `host.docker.internal`)

### Authenticated mode

If your Paperplane deployment is `authenticated`, provide auth context:

```bash
PAPERPLANE_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
PAPERPLANE_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```

### Network topology tips

- Local same-host smoke: default callback uses `http://127.0.0.1:<port>/webhook`.
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Paperplane server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Paperplane URL (typically `http://host.docker.internal:3100`).
- If Paperplane rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm paperplaneai allowed-hostname host.docker.internal
```

Then restart Paperplane and rerun the smoke script.
...
```bash
pnpm paperplaneai allowed-hostname <host>
```
