...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and Paperplane.
...
If `PAPERPLANE_APPROVAL_ID` is set, handle the approval first:
...
- If `PAPERPLANE_TASK_ID` is set and assigned to you, prioritize it
...
Headers: X-Paperplane-Run-Id: {runId}
