# ClipHub: Marketplace for Paperplane Team Configurations

> The "app store" for whole-company AI teams — pre-built Paperplane configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Paperplane-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Paperplane company (agents, projects, governance) |
...
A complete Paperplane company configuration:
...
Single-agent configurations designed to plug into a Paperplane org:
...
Portable skill files that any Paperplane agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Paperplane's skill loading system
...
interface Listing {
  ...
  paperplaneVersionMin: string;     // Minimum Paperplane version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Paperplane company
...
| `POST` | `/api/listings/:id/install` | Install to Paperplane company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Paperplane company (or create new)
  → ClipHub API calls Paperplane API to:
  ...
  → Redirect to Paperplane dashboard with new team running
```
...
### 5.3 Creator: Export from Paperplane → Publish

```
Running Paperplane company → "Export as Blueprint" (CLI or UI)
  → Paperplane exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Paperplane brand blue/purple)
...
| Install flow | One-click deploy to Paperplane company |
...
{
  "targetCompanyId": "uuid",         // Existing Paperplane company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `paperplane-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Paperplane
- **Backend**: Node.js API (or extend Paperplane server)
- **Database**: Postgres (can share Paperplane's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Paperplane auth (or OAuth2)

### 9.2 Integration with Paperplane

ClipHub can be:
- **Option A**: A separate app that calls Paperplane's API to install blueprints
- **Option B**: A built-in section of the Paperplane UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Paperplane UI and API.
...
- [ ] Install flow: blueprint → Paperplane company
...
- [ ] "Export from Paperplane" CLI command
