...
Paperplane supports two runtime modes with different security profiles.
...
```sh
pnpm paperplaneai allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Paperplane emits a one-time claim URL at startup:
...
```sh
PAPERPLANE_DEPLOYMENT_MODE=authenticated pnpm paperplaneai run
```
