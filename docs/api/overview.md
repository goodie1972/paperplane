...
Paperplane exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`PAPERPLANE_API_KEY`)
...
- Run audit trail: include `X-Paperplane-Run-Id` header on all mutating requests during heartbeats
